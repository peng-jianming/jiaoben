...
async function multiPointColorMatch(colorPoints, imagePath, tolerance = 10, searchRegion = null) {
    try {
        // 1. 使用 Jimp 加载图片
        ...
        const offsets = colorPoints.map(point => ({
            color: hexToBgr(point.颜色), // 注意：这里要转换为 BGR
        ...
        for (let y = startY; y < endY; y++) {
            for (let x = startX; x < endX; x++) {
                // 检查基准点颜色是否匹配
                const baseColor = getPixelColor(bgrMat, x, y);

                if (isColorMatch(baseColor, offsets[0].color, tolerance)) {
                    // 检查其他点是否匹配
                    let allMatch = true;
                    ...
                    for (let i = 1; i < offsets.length; i++) {
                        ...
                        const checkColor = getPixelColor(bgrMat, checkX, checkY);
                        if (!isColorMatch(checkColor, offsets[i].color, tolerance)) {
                            allMatch = false;
                            break;
                        }
                    }
                    ...
                    if (allMatch) {
                        // 调试信息
                        const debugColor = getPixelColor(bgrMat, x, y);
 ...
 * @returns {Object} BGR颜色对象
 */
function getPixelColor(mat, x, y) {
...
async function debugColorAtPoint(imagePath, x, y) {
    try {
        ...
        const color = getPixelColor(bgrMat, x, y);
